...
class Astronomical {
  ...
  static double apparentSolarLongitude(double T, double L0) {
    ...
    final double lambda= longitude - 0.00569 - (0.00478 * sin(toRadians(omega)));
  ...
  static double solarEquationOfTheCenter(double T, double M) {
    ...
    final double term1 = (1.914602 - (0.004817 * T) - (0.000014 * pow(T, 2))) * sin(Mrad);
  ...
  final double term2 = cos(toRadians(fai)) *
  cos(toRadians(delta)) * cos(toRadians(H));
  ...
  final double term2 = cos(toRadians(coordinates.latitude)) * cos(toRadians(delta2));
  ...
  final double term4 = 360 * cos(toRadians(delta)) *
