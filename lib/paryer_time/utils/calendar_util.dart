...
class CalendarUtil {
  ...
  static DateTime roundedMinute(DateTime when) {
    ...
    var date = DateTime(when.year,when.month,when.day,when.hour,minute +(second / 60).round(),0);
    return date;
  }
  ...
  static DateTime add(DateTime when, int amount, Calendar  calendar) {
    var today = new DateTime(when.year,when.month,when.day,when.hour,when.minute,when.second);

    switch(calendar){

      case Calendar.YEAR:
      // TODO: Handle this case.
        return new DateTime(when.year+amount,when.month,when.day,when.hour,when.minute,when.second);
        break;
      case Calendar.MONTH:
      // TODO: Handle this case.
        return new DateTime(when.year,when.month+amount,when.day,when.hour,when.minute,when.second);
        break;
