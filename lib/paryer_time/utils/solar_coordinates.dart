...
class SolarCoordinates {
  ...
  SolarCoordinates(double julianDay) {
    ...
    double lambda = Astronomical.toRadians(
    ...
    double talent = Astronomical.nutationInLongitude(/* julianCentury */ T, /* solarLongitude */ L0,
    /* lunarLongitude */ Lp, /* ascendingNode */ omega);
    double deltaepsilon = Astronomical.nutationInObliquity(/* julianCentury */ T, /* solarLongitude */ L0,
    /* lunarLongitude */ Lp, /* ascendingNode */ omega);

    double epsilon0 = Astronomical.meanObliquityOfTheEcliptic(/* julianCentury */ T);
    double epsilonapp = Astronomical.toRadians(Astronomical.apparentObliquityOfTheEcliptic(
    /* julianCentury */ T, /* meanObliquityOfTheEcliptic */ epsilon0));

    /* Equation from Astronomical Algorithms page 165 */
    this.declination = Astronomical.toDegrees(asin(sin(epsilonapp) * sin(lambda)));

    /* Equation from Astronomical Algorithms page 165 */
    var s = atan2(cos(epsilonapp) * sin(lambda), cos(lambda));
    this.rightAscension = DoubleUtil.unwindAngle(Astronomical.toDegrees(atan2(cos(epsilonapp) * sin(lambda), cos(lambda))));

    /* Equation from Astronomical Algorithms page 88 */
    this.apparentSiderealTime = theta0 + (((talent * 3600) * cos(Astronomical.toRadians(epsilon0 + deltaepsilon))) / 3600);
  }
}